...
int main ()
{
	...
	if (n == -1) {
		...
		n = recvfrom (fd, buff, sizeof (buff), MSG_CMSG_CLOEXEC, (struct sockaddr*) &client_addr, &client);
		if (n > 0) {
			printf ("received %s \n", buff);
		} else {
			printf ("failed \n");
		...
		n = sendto (fd, buff, sizeof (buff), MSG_CONFIRM, (struct sockaddr*) &client_addr, client);
		if (n > 0) {
			printf ("Send success \n");
		}
	}
